...
impl GemError {
    pub fn kind(&self) -> GemErrorKind {
        match self {
            GemError::ConnectionError(e) if e.is_timeout() => GemErrorKind::Timeout,
            GemError::ConnectionError(_) => GemErrorKind::Connection,
            GemError::GeminiAPIError(_) => GemErrorKind::Api,
            GemError::FileError(message)
                if message.to_ascii_lowercase().contains("timeout")
                    || message.to_ascii_lowercase().contains("timed out") =>
            {
                GemErrorKind::Timeout
            }
            GemError::FileError(_) => GemErrorKind::File,
            GemError::ParsingError(_) => GemErrorKind::Parsing,
            _ => GemErrorKind::Other,
        }
    }

    pub fn is_timeout(&self) -> bool {
        self.kind() == GemErrorKind::Timeout
    }
}
